...
@click.option(
    ...
    "--ref",
    required=True,
    help="FASTA reference to generate mutations against",
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
)
@click.option(
    "-m",
    "--mutations",
    help="The mutations to generate a VCF for",
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
    required=True,
)
@click.option(
...
def main(ref, gff, mutations, out_vcf):
    reference = ""
    chrom = "CHROM"
    with open(ref) as fp:
    ...
    with open(mutations) as fp:
        for line in map(str.rstrip, fp):
            ...
            VCF_METALINES = f"""##fileformat=VCFv4.3
##FILTER=<ID=PASS,Description="All filters passed">
##FORMAT=<ID=GT,Number=1,Type=String,Description="Genotype">
##contig=<ID={chrom},length={len(reference)}>"""
            ...
            cmd = f"bcftools norm --check-ref e -f {ref} -o {norm_vcf} {tmpvcf}"
    ...
    cmd = f"bcftools norm --check-ref e -f {ref} -o {norm_vcf} {merged_orphan_vcf}"
    args = shlex.split(cmd)
    cp = subprocess.run(args, capture_output=True, text=True)
    if cp.returncode != 0:
